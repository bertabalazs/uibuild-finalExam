...
const displayBooks = (index, title, sub, text) => {
    return`
    <div class="card">
			...
			<p class="sub">${sub}</p>
                ...
                <p class="text">${text}</p>
                <button ><span class="readMore">Read more</span><span class="material-symbols-outlined" id="arrowIcon">arrow_forward</span></button>
            </div>
        </div>
...
const bookElements = ({cards}) => {
    return cards.map((card, index) => displayBooks(index + 1, card.title, card.sub, card.text)).join(' ')
}

const header = `
<div class="header">
    <h1>New york times best sellers</h1>
    <button class="material-icons"><span class="material-symbols-outlined">
	menu
	</span></button>
</div>
`
const loadEvent = () => {
	...
	rootElement.insertAdjacentHTML("beforeend", bookElements(cards))
